...
export default class Authentication {
  ...
  static UserKey(user){
    return '@skipapp_profile:' + user.email;
  }

  static LoadProfile(dispatch, _this){
    return new Promise((resolve, reject) => {
      AsyncStorage.getItem(Authentication.UserKey(_this.props.user))
      .then(strProfile =>{
        if (strProfile){
  ...
  static SaveProfile(dispatch, _this){
    return new Promise((resolve, reject) => {
      ...
      AsyncStorage.setItem(Authentication.UserKey(_this.props.user), JSON.stringify(profile))
      .then(() => {
